...
double LinearDependencyFunction::getRate(std::vector<Parameter*> params, size_t state) const{
  double func_res = params[0]->getValue() + (((double)(state))*params[1]->getValue()); //TODO: check why it's state-1
  if (func_res < 0){
    return 0;
  }
  return func_res;
...
double ExponentialDependencyFunction::getRate(std::vector<Parameter*> params, size_t state) const{
  return params[0]->getValue() * std::exp(((double)(state))*params[1]->getValue()); //TODO: check why it's state-1
}
